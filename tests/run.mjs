...
const names = await page.locator('.hname-t').allInnerTexts();
...
{   // отдельная область: имена не конфликтуют с остальным прогоном
  ...
  const shownOrder = await page.locator('.hname-t').allInnerTexts();
...
/* ═══════════════ 24. Значок — эмодзи ═══════════════ */
{
  group('24. Значки-эмодзи');
  await setState(seed());
  await page.waitForTimeout(600);

  const quick = await page.evaluate(() => {
    const A = window.__askeza;
    const one = s => [...new Intl.Segmenter('ru', { granularity: 'grapheme' }).segment(s)].length === 1;
    return { n: A.EMOJI_QUICK.length, uniq: new Set(A.EMOJI_QUICK).size,
             multi: A.EMOJI_QUICK.filter(e => !one(e)) };
  });
  ok('частых эмодзи не меньше сорока', quick.n >= 40, String(quick.n));
  eq('повторов в списке нет', quick.uniq, quick.n);
  eq('каждое — один символ', quick.multi.length, 0, quick.multi.join(' '));

  eq('у курения эмодзи по типу',
     await page.evaluate(() => window.__askeza.emojiOf(window.__askeza.active())), '🚭');
  eq('свой эмодзи важнее типа',
     await page.evaluate(() => window.__askeza.emojiOf({ kind: 'smoking', emoji: '🔥' })), '🔥');
  eq('старый ключ значка из v3.0 переехал в эмодзи',
     await page.evaluate(() => {
       const A = window.__askeza;
       A.set({ v: 2, activeId: 'z1', habits: [{ id: 'z1', name: 'Зал', kind: 'custom',
         icon: 'dumbbell', startedAt: new Date(Date.now() - 86400e3).toISOString(), history: [] }] });
       return A.S.habits[0].emoji;
     }), '🏋️');

  // склеенные эмодзи не разваливаются
  const graph = await page.evaluate(() => {
    const g = window.__askeza.lastGrapheme;
    return { flag: g('🏴‍☠️'), family: g('👨‍👩‍👧'), skin: g('🏋️‍♀️'), last: g('ab🔥'), empty: g('   ') };
  });
  eq('флаг остался целым', graph.flag, '🏴‍☠️');
  eq('семья осталась целой', graph.family, '👨‍👩‍👧');
  eq('модификатор не отвалился', graph.skin, '🏋️‍♀️');
  eq('берётся последний символ', graph.last, '🔥');
  eq('из пробелов ничего не берётся', graph.empty, '');

  await setState(seed());
  await page.waitForTimeout(500);
  ok('эмодзи виден на экране привычки', (await page.locator('.h-ico').innerText()).includes('🚭'));
  await page.click('[data-act="backToList"]');
  await page.waitForTimeout(500);
  ...
  await page.waitForSelector('#p-add.on');
  await page.waitForTimeout(400);
  eq('до выбора типа значка нет', await page.locator('#a-emoji').count(), 0);
  await page.click('[data-act="pickKind"][data-arg="custom"]');
  await page.waitForTimeout(450);
  eq('поле подставило эмодзи типа', await page.locator('#a-emoji').inputValue(), '✨');
  const gridN = await page.locator('#p-add .em').count();
  ok('сетка частых на месте', gridN >= 40, String(gridN));
  await page.fill('#a-name', 'Зал');
  await page.click('[data-act="pickEmoji"][data-arg="🏋️"]');
  await page.waitForTimeout(250);
  eq('тап по сетке попал в поле', await page.locator('#a-emoji').inputValue(), '🏋️');
  eq('в сетке подсвечен ровно один', await page.locator('#p-add .em.on').count(), 1);

  // любое эмодзи с клавиатуры — вводим прямо в поле
  await page.fill('#a-emoji', '🦄');
  await page.waitForTimeout(250);
  eq('своё эмодзи принято', await page.evaluate(() => window.__askeza.S && document.getElementById('a-emoji').value), '🦄');
  eq('его нет в сетке — ничего не подсвечено', await page.locator('#p-add .em.on').count(), 0);
  await page.fill('#a-emoji', 'привет🔥');
  await page.waitForTimeout(250);
  eq('из строки берётся последний символ', await page.locator('#a-emoji').inputValue(), '🔥');
  await page.screenshot({ path: path.join(SHOTS, '34-emoji-picker.png') });
  await page.click('[data-act="quickWhen"][data-arg="morning"]');
  await page.waitForTimeout(300);
  await page.click('[data-act="saveNew"]');
  await page.waitForTimeout(900);
  eq('эмодзи сохранился', await page.evaluate(() => window.__askeza.active().emoji), '🔥');

  // смена в «Изменить»
  await page.click('[data-act="habitMenu"]'); await page.waitForTimeout(420);
  await page.click('[data-act="edit"]'); await page.waitForSelector('#p-add.on');
  await page.waitForTimeout(400);
  eq('в правке подставлен текущий', await page.locator('#e-emoji').inputValue(), '🔥');
  await page.fill('#e-emoji', '🏴‍☠️');
  await page.waitForTimeout(250);
  await page.click('[data-act="saveEdit"]');
  await page.waitForTimeout(900);
  eq('склеенный эмодзи сохранился целиком',
     await page.evaluate(() => window.__askeza.active().emoji), '🏴‍☠️');

  await page.reload(); await page.waitForFunction(() => !!window.__askeza);
  await page.waitForTimeout(700);
  eq('пережил перезагрузку',
     await page.evaluate(() => window.__askeza.S.habits.find(h => h.name === 'Зал').emoji), '🏴‍☠️');

  // ссылка может нести эмодзи, а инъекция через него не проходит
  const seedEmoji = Buffer.from(JSON.stringify({ habits: [
    { name: 'Бег', kind: 'custom', emoji: '🏃', startedAt: '2026-01-01T08:00:00' } ] }), 'utf8')
    .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  eq('ссылка несёт эмодзи',
     await page.evaluate(h => window.__askeza.parseSeed('#seed=' + h)[0].emoji, seedEmoji), '🏃');
  await page.evaluate(() => {
    const A = window.__askeza;
    A.set({ v: 2, activeId: 'x1', habits: [{ id: 'x1', name: 'Тест', kind: 'custom',
      emoji: '<img src=x onerror=window.__XSS2=1>', startedAt: new Date(Date.now() - 86400e3).toISOString(), history: [] }] });
    A.goList();
  });
  await page.waitForTimeout(600);
  ok('инъекция через значок не проходит', await page.evaluate(() => !window.__XSS2));
